...
export default class Model {
	...
	initPoints() {
		this.loader.load(this.file, (gltf) => {
			...
			const geometries = [] // Array to store individual geometries
			...
			for (const mesh of meshes) {
				const points = this.createPoints(mesh)
				const geometry = points.geometry.clone() // Clone to avoid modifying the original

				// Apply the mesh's world transformations to the geometry
				...
				geometries.push(geometry) // Collect transformed geometries
			}

			// Merge all geometries into one
			const mergedGeometry = mergeGeometries(geometries, true)
			const mergedPoints = new Points(
				mergedGeometry,
				new PointsMaterial({
					vertexColors: true,
					transparent: true,
